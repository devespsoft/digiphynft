...
export default class product extends Component {
                ...
                cell: (item) => {
                    return (
                        <>
                            ...
                            {(item.is_on_sale == 0) && (item.owner_id == this.loginData?.data?.id) ?
                                <button onClick={this.updateonMarket.bind(this, item)} style={{ left: '200px', padding: '1px', marginBottom: '4px' }} className='button_Edit btn btn-primary' type='submit' >Put on sale
                                </button> : ''}
                        </>
                    );
                }
        ...
        axios.get(`${config.apiUrl}getAdminItem`, {},)
            .then(response => {
                if (response.data.success === true) {
                    this.setState({
                        item_list: response.data.response.filter(item => item.created_by == 1 && item.bulk_nft_master_id == 0)
                    })
    ...
    async updateonMarket(item) {
        ...
        confirmAlert({
            title: 'Confirm to submit',
            message: 'Are you sure to put on sale of this NFT!',
            buttons: [
                {
                    label: 'Yes',
                    onClick: async () =>
                        await axios({
                            ...
                            data: { 'item_id': item.id, 'user_id': this.loginData.data.id, 'item_edition_id': item.item_edition_id, 'quantity': item.quantity, 'price': item.price }
                        }).then(response => {
                            if (response.data.success === true) {
                                this.setState({
                                    isDialogOpen: false,
                        ...
                        <div className="container-fluid">
                            ...
                            <div className="row heading-bg">
                                <div className="col-lg-3 col-md-4 col-sm-4 col-xs-12">
                                    <h5 className="" style={{ color: '#fff' }}>add-products</h5>
                                </div>
                                {/* <!-- Breadcrumb --> */}
                                    ...
                                    <div className="panel panel-default card-view" id="users">
                                        <div className="panel-wrapper collapse in">
                                            <div className="panel-body">
                                                <a href={`${config.baseUrl}create_an_item`}>
                                                    <button type='button' className="btn btn-primary pb-4">Add NFTs </button></a>
                                                <br />
                                                <br />
