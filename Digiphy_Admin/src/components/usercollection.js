...
<button style={{ padding: '3px' }} type='button' onClick={this.approveNFTAdmin.bind(this, item)} className="btn btn-primary">Approve</button>
    ...
    async approveNFTAdmin(item) {
        ...
        await axios({
            ...
            data: { 'item_id': item.collection_id
        }
        }).then(response => {
            if (response.data.success === true) {

                toast.success('Collection is Approved Now!');
                setTimeout(() => {
    ...
    async nftsuserList() {
        await axios.get(`${config.apiUrl}getAdminAllUserCollection`, {},)
            .then(result => {

                if (result.data.success === true) {
                    this.setState({
                        user_list: result.data.response.filter(item => item.user_id != 1)
                    })
                        ...
                        <div className="container-fluid">
                            ...
                            <div className="row heading-bg">
                                <div className="col-lg-3 col-md-4 col-sm-4 col-xs-12">
                                    <h5 className="">User Collection</h5>
                                </div>
                                {/* <!-- Breadcrumb --> */}
