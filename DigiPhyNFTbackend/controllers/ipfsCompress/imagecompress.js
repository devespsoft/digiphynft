...
console.log("in image compress");
    ...
    try {
        ...
        for (var i = 0; i < ipfsImages.length; i++) {
            console.log("in for loop 68");
            var img = ipfsImages[i];
            // let result = await probe(img);
            let options = { responseType: 'base64',jpegOptions: { force: true, quality: 50 } }
            ...
            if (buffer.byteLength > 2097152 ) 
            {
                ...
                console.log("in compress else block");
        ...
        return { success: false, imageHash: mainArray,images:imagesArr }
    }
}
